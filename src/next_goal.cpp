...
float x_current, last_x_current;
float y_current, last_y_current;
...
int main(int argc, char *argv[])
{
  ...
  ros::Subscriber sub1 = next_goal.subscribe("/odom", 1, pose_callback);
  ...
  while (ros::ok())
  {
    ...
    if (!planned_path.Path.empty())
    {
      ...
      else if (sqrt(pow(last_x_current - x_current, 2) + pow(last_y_current - y_current, 2)) <= 1e-3)
      {
          count ++;
          goal_reached = false;
